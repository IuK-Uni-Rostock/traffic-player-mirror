...
class Generator:
    ...
    def generate(self, types, target, duration, workload):
        ...
        for i in range(1, amount):
            result.append(self.__create_telegram(self._rng.choice(types), target))
        return result

    def __create_telegram(self, telegram_type, target):
        t = Telegram()
        ...
        elif target == 'LED Strips':
            if is_group_address:
                strip_group_addresses = ['1/1/1', '1/1/3']
                t.destination_addr = self._rng.choice(strip_group_addresses)
        #    else: TOOD: figure out LED strip physical addresses
        #        strip_addresses = ['']
        #        t.destination_addr = self._rng.choice(strip_addresses)
